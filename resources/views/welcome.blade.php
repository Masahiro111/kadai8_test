                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left transition-all sm:my-0 sm:w-full sm:max-w-lg sm:p-6">
                    <div class="sm:flex sm:items-start">
                        <div class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full sm:mx-0 sm:h-12 sm:w-12">
                            ...
                            <img class="h-12 w-12 mt-2 rounded-full object-cover" src="{{ $article->user->profile_photo_url }}" alt="{{ $article->user->name }}">
                        </div>
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <div class="mb-2">
                                <p class="text-sm text-gray-500 mb-1">&#064;{{ $article->user->name }}</p>
                                <p class="text-xs text-gray-500">{{ $article->created_at }}</p>
                            </div>
                            <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">{{ $article->title }}</h3>
                            {{-- <div class="mt-2">
